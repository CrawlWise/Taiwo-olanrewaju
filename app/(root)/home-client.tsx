...
export default function HomeClient({ initialBlogPosts, initialFreeBook }: HomeClientProps) {
  const [blogPosts] = useState<any[]>(initialBlogPosts || []);
  const [freeBook] = useState<any>(initialFreeBook || null);
              ...
              {freeBook ? (
                downloadSubmitted ? (
                  <motion.div 
                    ...
                    className="space-y-5 bg-white/10 p-8 rounded-3xl border border-white/15 w-full max-w-md backdrop-blur-md text-center shadow-2xl"
                  >
                    <div className="mx-auto w-16 h-16 bg-gold/15 rounded-full flex items-center justify-center text-gold border border-gold/30">
                      <ShieldCheck className="w-9 h-9" />
                    </div>
                    <div className="space-y-2">
                      <h3 className="text-2xl font-bold text-white font-poppins tracking-tight">eBook Request Sent!</h3>
                      <p className="text-sm text-white/80 leading-relaxed">
                        We have sent your copy of <br />
                        <span className="text-gold font-semibold">"{freeBook.title}"</span> <br />
                        to <span className="underline decoration-gold decoration-2 text-white font-medium">{downloadEmail}</span>.
                      </p>
                    </div>
                    <div className="pt-2 border-t border-white/5">
                      <p className="text-xs text-white/50 leading-relaxed">
                        Please check your inbox (and spam or promotions folder) in a few minutes to download the PDF.
                      </p>
                    </div>
                  </motion.div>
                ) : (
